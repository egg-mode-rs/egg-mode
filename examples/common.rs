...
pub struct Config {
    pub con_token: twitter::Token<'static>,
    pub access_token: twitter::Token<'static>,
    pub user_id: i64,
...
impl Config {
    pub fn load() -> Self {
        ...
        if let Ok(mut f) = std::fs::File::open("twitter_settings") {
            ...
            access_token = twitter::Token::new(iter.next().unwrap().to_string(),
                                                     iter.next().unwrap().to_string());

            println!("Welcome back, {}!", username);
        }
        else {
            let request_token = twitter::request_token(&token, "oob").unwrap();
