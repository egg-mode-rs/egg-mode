...
pub fn upload_image<'a>(image: &[u8], token: &auth::Token, handle: &Handle) -> FutureResponse<Media> {
...
pub mod upload {
    ...
    pub fn init<'a>(total: usize, mime: mime::Mime, token: &auth::Token, handle: &Handle) -> FutureResponse<Media> {
        let mut params = HashMap::new();
    ...
    pub fn append<'a>(id: u64, chunk: &[u8], index: usize, token: &auth::Token, handle: &Handle) -> FutureResponse<()> {
        let mut params = HashMap::new();
    ...
    ///
    ///* id - Media's id returned in response to `INIT` message.
    pub fn finalize<'a>(id: u64, token: &auth::Token, handle: &Handle) -> FutureResponse<Media> {
        let mut params = HashMap::new();
    ...
    pub fn status<'a>(id: u64, token: &auth::Token, handle: &Handle) -> FutureResponse<Media> {
        let mut params = HashMap::new();
